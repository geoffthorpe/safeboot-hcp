...
pristine_submodules:
	$Qecho "Cleaning submodules"
	$Qgit submodule foreach git reset --hard
	$Qgit submodule foreach git clean -f -d -x

include settings.mk

HCP_OUT ?= $(TOP)/output/$(HCP_VARIANT)
HCP_SRC := $(TOP)/src
HCP_UTIL := $(TOP)/util
...
FOO := $(HCP_OUT)/docker-compose.env
$(shell echo "## Autogenerated environment, used by docker-compose.yml" > $(FOO).tmp)
$(foreach i,$(HCP_DOCKER_IMGS),\
$(shell echo "HCP_IMAGE_$i=$(call HCP_IMAGE_FN,$i,$(HCP_VARIANT))" >> $(FOO).tmp)\
$(foreach j,$(HCP_VARIANT_LIST),\
$(shell echo "HCP_IMAGE_$i_$j=$(call HCP_IMAGE_FN,$i,$j)" >> $(FOO).tmp)))
